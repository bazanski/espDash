...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len) {
  ...
  uint16_t seq = 0;
  const EspDashTelemetry *pkt =
      espdash_parse(incomingData, len, &payload_len, &seq);
  if (!pkt)
    return;
...
static void node_cmd_tick(uint32_t now) {
  ...
  if (now - last_send < ESPDASH_NODE_CMD_INTERVAL_MS)
    return;
  last_send = now;
...
static volatile uint32_t dbg_t_btn = 0, dbg_t_tick = 0, dbg_t_ui = 0,
                         dbg_t_lock = 0;
#endif
...
static void IRAM_ATTR rec_button_isr(void) {
  ...
  uint32_t t = millis();
  if (t - isr_last_press_ms < REC_DEBOUNCE_MS)
    return; // bounce
  isr_last_press_ms = t;
...
static void rec_button_init(void) {
  ...
  attachInterrupt(digitalPinToInterrupt(REC_BUTTON_PIN), rec_button_isr,
                  FALLING);
}
...
static void rec_label_update(void) {
  ...
  if (millis() >= dbg_next) {
    ...
    Serial.printf("[UI] status_label=%p warning_label=%p state=%d\n",
                  (void *)objects.status_label, (void *)objects.warning_label,
                  (int)st);
  }
#endif
  ...
  if (objects.status_label) {
    ...
    if (st == CANLOG_RECORDING) {
      ...
      if (wr < 1024UL * 1024UL) {
        snprintf(buf, sizeof(buf), LV_SYMBOL_STOP " REC %04u  %lu:%02lu  %luKB",
                 (unsigned)canlog_file_index(), (unsigned long)(sec / 60),
                 (unsigned long)(sec % 60), (unsigned long)(wr / 1024));
      } else {
        snprintf(
            buf, sizeof(buf), LV_SYMBOL_STOP " REC %04u  %lu:%02lu  %lu.%luMB",
            (unsigned)canlog_file_index(), (unsigned long)(sec / 60),
            (unsigned long)(sec % 60), (unsigned long)(wr / (1024UL * 1024UL)),
    ...
    lv_obj_set_style_text_color(objects.status_label, lv_color_hex(colour),
                                LV_PART_MAIN);
  ...
  if (objects.warning_label) {
    ...
    if (st == CANLOG_ERROR) {
      ...
      lv_obj_set_style_text_color(objects.warning_label, lv_color_hex(0xff4040),
                                  LV_PART_MAIN);
      ...
      lv_obj_set_style_text_color(objects.warning_label, lv_color_hex(0xffa000),
                                  LV_PART_MAIN);
...
void setup() {
  ...
  if (lvgl_port_lock(500)) {
    ...
    if (objects.main) {
      lv_obj_set_style_bg_color(objects.main, lv_color_hex(0x0f111a),
                                LV_PART_MAIN);
    ...
    if (objects.history_chart) {
      ...
      lv_chart_set_update_mode(objects.history_chart,
                               LV_CHART_UPDATE_MODE_SHIFT);
      lv_chart_set_range(objects.history_chart, LV_CHART_AXIS_PRIMARY_Y, 0,
                         100);
      lv_chart_set_range(objects.history_chart, LV_CHART_AXIS_SECONDARY_Y, 0,
                         60);
      lv_chart_set_point_count(
          objects.history_chart,
          200); // 200 samples @ 20Hz = 10 sec rolling window
      ...
      lv_obj_set_style_bg_color(objects.history_chart, lv_color_hex(0x141824),
                                LV_PART_MAIN);
      lv_obj_set_style_border_color(objects.history_chart,
                                    lv_color_hex(0x2a2f45), LV_PART_MAIN);
      lv_obj_set_style_line_width(objects.history_chart, 2, LV_PART_ITEMS);

      // Add 3 Time-Synchronized Series
      ser_throttle = lv_chart_add_series(
          objects.history_chart, lv_color_hex(0x00FF00),
          LV_CHART_AXIS_PRIMARY_Y); // Neon Green (Throttle %)
      ser_brake =
          lv_chart_add_series(objects.history_chart, lv_color_hex(0xFF0000),
                              LV_CHART_AXIS_PRIMARY_Y); // Bright Red (Brake %)
      ser_speed = lv_chart_add_series(
          objects.history_chart, lv_color_hex(0xFFFF00),
          LV_CHART_AXIS_SECONDARY_Y); // Bright Yellow (Speed KM/H)
    ...
    if (objects.throttle_bar) {
      ...
      lv_obj_set_style_bg_color(objects.throttle_bar, lv_color_hex(0x00FF00),
                                LV_PART_INDICATOR);
    }
    if (objects.brake_bar) {
      lv_bar_set_range(objects.brake_bar, 0, 100);
      lv_obj_set_style_bg_color(objects.brake_bar, lv_color_hex(0xFF0000),
                                LV_PART_INDICATOR);
    }
    if (objects.rpm_bar) {
      lv_bar_set_range(objects.rpm_bar, 0, 7000);
      lv_obj_set_style_bg_color(objects.rpm_bar, lv_color_hex(0x00BFFF),
                                LV_PART_INDICATOR);
    }
  ...
  if (esp_now_init() == ESP_OK) {
    ...
    if (esp_now_add_peer(&peer) != ESP_OK) {
      Serial.println(
          "[ESP-NOW] Failed to add broadcast peer - cannot arm gateway!");
    }
  }
}
...
void loop() {
...
#if REC_BUTTON_DEBUG
  {
    ...
    if (now >= hb) {
      ...
      Serial.printf("[BTN] presses=%lu toggles=%lu ok=%lu fail=%lu "
                    "state=%d file=%04u sd=%s tlm=%lu clog=%lu tx=%lu "
                    "txfail=%lu(%d) loop=%lums\n",
                    ...
                    sdcard_status_str(), (unsigned long)pkt_count,
                    (unsigned long)canlog_packets_rx(),
  ...
  LinkState link =
      live ? LINK_LIVE : (ever_linked ? LINK_LOST : LINK_SEARCHING);
  ...
  if (is_demo) {
    ...
    float thr_val =
        50.0f + 42.0f * sin(phase * 1.3f) + 14.0f * sin(phase * 2.9f);
    float brk_val =
        45.0f + 42.0f * sin(phase * 1.3f + 2.4f) + 12.0f * cos(phase * 3.3f);

    active_pkt.throttle_pct = (uint8_t)constrain((int)thr_val, 0, 100);
    active_pkt.brake_pct = (uint8_t)constrain((int)brk_val, 0, 100);
    active_pkt.rpm = (uint16_t)constrain(
        (int)(1200 + (active_pkt.throttle_pct / 100.0f) * 5200 +
              sin(phase * 4.2f) * 350),
        800, 7000);
    active_pkt.speed_kmh_x10 =
        (uint16_t)((35 + (active_pkt.throttle_pct / 100.0f) * 65 +
                    sin(phase * 0.8f) * 20) *
  ...
  if (now - last_ui_update >= 50) {
    ...
    if (lvgl_port_lock(10)) {
...
#endif
      // Chart feed with dynamic speed autoscale (visible 10s window max + 10
      // km/h padding, baseline 60 km/h)
      if (objects.history_chart && ser_throttle && ser_brake && ser_speed) {
        uint16_t spd_kmh = active_pkt.speed_kmh_x10 / 10;

        // Store in circular buffer matching the 200 visible chart points (10
        // sec)
        ...
        // Baseline Y-max is 60 km/h. When speed exceeds 50 km/h, scale to
        // (visible_max + 10 km/h) so the yellow speed line never touches the
        // top border of the chart. Reverts back to 60 km/h automatically once
        // high speed points roll off the screen.
        uint16_t target_ymax =
            (visible_max_speed > 50) ? (visible_max_speed + 10) : 60;

        if (target_ymax != current_speed_ymax) {
          current_speed_ymax = target_ymax;
          lv_chart_set_range(objects.history_chart, LV_CHART_AXIS_SECONDARY_Y,
                             0, current_speed_ymax);
        }

        lv_chart_set_next_value(objects.history_chart, ser_throttle,
                                active_pkt.throttle_pct);
        lv_chart_set_next_value(objects.history_chart, ser_brake,
                                active_pkt.brake_pct);
      ...
      if (objects.throttle_bar && active_pkt.throttle_pct != prev_throttle) {
        lv_bar_set_value(objects.throttle_bar, active_pkt.throttle_pct,
                         LV_ANIM_OFF);
