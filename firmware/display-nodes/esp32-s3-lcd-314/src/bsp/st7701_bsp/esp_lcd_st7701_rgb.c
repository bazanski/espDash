...
static st7701_panel_t *g_st7701_rgb = NULL;
                                       ...
                                       esp_lcd_panel_handle_t *ret_panel)
{
    ...
    st7701->disp_on_off = (*ret_panel)->disp_off;
    ...
    (*ret_panel)->disp_off = panel_st7701_disp_on_off;
...
static esp_err_t panel_st7701_init(esp_lcd_panel_t *panel)
{
    st7701_panel_t *st7701 = g_st7701_rgb;
...
static esp_err_t panel_st7701_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    if (st7701->flags.display_on_off_use_cmd) {
        if (io == NULL) {
            ESP_LOGW(TAG, "Panel IO is deleted, skipping display on/off command");
        } else {
...
void release_st7701_spi_pins(void)
{
    if (g_st7701_rgb && g_st7701_rgb->io) {
        esp_lcd_panel_io_del(g_st7701_rgb->io);
        g_st7701_rgb->io = NULL;
        ESP_LOGI(TAG, "Released 3-wire SPI pins");
    }
}
#endif
